...
import com.example.camera.cameralibrary.listener.CaptureListener;
import com.example.camera.cameralibrary.listener.ClickListener;
import com.example.camera.cameralibrary.listener.ErrorListener;
import com.example.camera.cameralibrary.listener.JCameraListener;
import com.example.camera.cameralibrary.listener.TypeListener;
import com.example.camera.cameralibrary.ottoutils.SendNews;
import com.example.camera.cameralibrary.state.CameraMachine;
import com.example.camera.cameralibrary.util.FileUtil;
import com.example.camera.cameralibrary.util.LogUtil;
import com.example.camera.cameralibrary.util.ScreenUtils;
import com.example.camera.cameralibrary.view.CameraView;
        ...
        mCaptureLayout.setLeftClickListener(new ClickListener() {
            ...
            public void onClick() {
                if (leftClickListener != null) {
                    ...
                    AppBus.getInstance().post(new SendNews(videoUrl,"3"));
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
        ...
        switch (type) {
            case TYPE_VIDEO:
                ...
                if (jCameraLisenter != null) {
                    ...
                    //部分机型jCameraLisenter为null
                    AppBus.getInstance().post(new SendNews(videoUrl,"1"));
            ...
            case TYPE_PICTURE:
                ...
                if (jCameraLisenter != null) {
                    ...
                    jCameraLisenter.captureSuccess(pathimg);
                }else{
                    //部分机型jCameraLisenter为null

                    AppBus.getInstance().post(new SendNews(pathimg,"2"));
     ...
     * @return
     */
    public String saveBitmapToSD(Bitmap bt) {
        if (bt == null) {
            return "";
        }
